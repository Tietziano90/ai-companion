...
import {
  ...
  stopStreaming,
  interruptSpeaking,
} from "@/services/heygenProxy";
import { useConversationStore } from "@/stores/conversationStore";
import { useVisualOverlayStore } from "@/stores/visualOverlayStore";
import {
  parseRichResponse,
  type ParsedResponse,
} from "@/lib/richResponseParser";
...
export function useAvatarConversation() {
  ...
  const initializeAvatar = useCallback(
    async (videoElement: HTMLVideoElement) => {
      try {
        ...
        console.log(
          "Avatar session created and streaming started:",
          sessionInfo
        );
  ...
  const speakViaProxy = useCallback(
    async (text: string) => {
      ...
      if (avatarRef.current) {
        ...
        try {
          ...
          console.error(
            "[HeyGen SDK] speak failed, falling back to proxy:",
            error
          );
        }
      }
      ...
      if (tokenRef.current && heygenSessionRef.current) {
        ...
        try {
          ...
          console.error(
            "[HeyGen] speak failed, falling back to browser TTS:",
            error
          );
        }
      }
  ...
  const startConversation = useCallback(
    async (videoElement?: HTMLVideoElement | null) => {
      ...
      try {
        ...
        const { sessionId: newSessionId, welcomeMessage } =
        ...
        if (videoElement) {
          try {
            ...
            console.error(
              "[HeyGen] avatar init failed (continuing with Agentforce):",
              avatarError
            );
            toast.warning(
              "Avatar is temporarily unavailable; continuing with Agentforce."
            );
          }
        }
      ...
      try {
        if (demoMode) {
          ...
          demoIndexRef.current =
            (demoIndexRef.current + 1) % DEMO_RESPONSES.length;
        ...
        const { message, progressIndicators } = await sendAgentMessage(
          sessionId,
          text
        );
        setLastAgentforceResponse(message || "");
        console.log("[Agentforce] received response", {
        ...
        if (message) {
          ...
          console.log(
            "[HeyGen] speaking Agentforce response",
            parsed.speechText.substring(0, 80) + "..."
          );
          try {
            await speakViaProxy(parsed.speechText);
